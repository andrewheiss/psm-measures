...
```{r load-data}
psm_raw_spss <- read_spss(file.path(root, "data", "data_raw", "Student PSM Survey Data.sav")) %>% 
  zap_formats() %>% zap_widths()

psm_raw_excel <- read_excel(file.path(root, "data", "data_raw", "Student PSM Survey Master Data.xlsx"),
                            sheet = "Master Data")
```
...
psm_codebook_simple <- read_excel(file.path(root, "data", "data_raw", "psm measures_1.xls"),
...
```{r save-clean-data}
write_csv(psm_indexes, file.path(root, "data", "data_clean", "psm_clean.csv"))
```
